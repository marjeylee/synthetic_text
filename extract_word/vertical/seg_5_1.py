...
def_path = 'E:\dataset/text_area/vertical/vertical_seg\seg_5_1/'
...
def seg_vertical(label, img):
    ...
    if len(label) != size:
        return
    tmp_img = img[0:int((text_array[4][-1] + text_array[5][0]) / 2), :, :]
    cv2.imwrite(def_path + label[:5] + '-' + str(uuid.uuid4()) + '.jpg', tmp_img)
    tmp_img = img[int((text_array[4][-1] + text_array[5][0]) / 2):, :, :]
    cv2.imwrite(def_path + label[5:] + '-' + str(uuid.uuid4()) + '.jpg', tmp_img)


def main():
    dir_path = 'E:\dataset/text_area/vertical/vertical_orig/'
